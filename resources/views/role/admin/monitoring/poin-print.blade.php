...
<head>
    <meta charset="UTF-8" />
    <title>Laporan {{ $label }} — {{ $group->name ?? '-' }}</title>
    <style>
        * { box-sizing: border-box; }
        ...
        table.data td.ket { text-align: left; }
...
<body>
    ...
    <!-- ============================================================
         ►► KOP SURAT — format resmi Universitas La Tansa Mashiro (UNILAM),
            sama persis dengan kop surat di absensi-print.blade.php.
    ============================================================= -->
    <div class="kop">
    ...
    <h3 class="judul">Laporan {{ $label }} Kelompok</h3>
    <p class="subjudul">Dicetak sebagai arsip resmi rekap {{ strtolower($label) }} mahasiswa</p>

    <table class="meta">
        <tr><td class="label">Kelompok</td><td>: {{ $group->name ?? '-' }}</td></tr>
        <tr><td class="label">Mentor</td><td>: {{ $group->mentor->name ?? '-' }}</td></tr>
        <tr><td class="label">Tanggal Cetak</td><td>: {{ now()->translatedFormat('d F Y') }}</td></tr>
    </table>
    ...
    <table class="data">
        <thead>
            <tr>
                ...
                <th style="width:90px">Poin {{ $label }}</th>
                <th style="width:100px">Update Terakhir</th>
        ...
        <tbody>
            @forelse ($rows as $idx => $r)
                <tr>
                    ...
                    <td class="center">{{ $r['poin'] }}</td>
                    <td class="center">{{ $r['update'] ? \Carbon\Carbon::parse($r['update'])->translatedFormat('d M Y') : '-' }}</td>
    ...
    <p class="keterangan">
        Keterangan: Poin {{ strtolower($label) }} dihitung dari akumulasi seluruh catatan
        {{ $label === 'Keaktifan' ? 'poin plus' : 'poin minus' }} mahasiswa yang tercatat oleh mentor.
    </p>
