...
<div class="mb-5 flex items-start justify-between flex-wrap gap-3">
    <div>
        <a href="{{ route($monBase.'.tugas') }}" class="text-sm font-semibold text-teal-600 inline-flex items-center gap-1 mb-3">
    ...
    <a href="{{ route($monBase.'.tugas.export-excel', ['groupId' => $group->id]) }}"
        ...
        <table class="w-full">
            <thead>
                <tr>
                    ...
                    @endphp
                    @foreach (['individu', 'kelompok'] as $tipe)
                        @if (($groupedTasks[$tipe] ?? collect())->count())
                            <th colspan="{{ $groupedTasks[$tipe]->count() }}"
                                class="text-center text-[10px] font-extrabold uppercase tracking-wider px-3.5 py-2 whitespace-nowrap {{ $tipe === 'kelompok' ? 'bg-teal-50 text-teal-600' : 'bg-indigo-50 text-indigo-600' }}">
