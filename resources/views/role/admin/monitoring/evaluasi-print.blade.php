...
<head>
    <meta charset="UTF-8" />
    <title>Laporan Evaluasi — {{ $group->name ?? '-' }}</title>
    <style>
...
<body>
    ...
    <h3 class="judul">Laporan Evaluasi Kelompok</h3>
    <p class="subjudul">Dicetak sebagai arsip resmi hasil evaluasi mahasiswa</p>

    <table class="meta">
        <tr><td class="label">Kelompok</td><td>: {{ $group->name ?? '-' }}</td></tr>
        <tr><td class="label">Mentor</td><td>: {{ $group->mentor->name ?? '-' }}</td></tr>
        <tr><td class="label">Jumlah Paket Evaluasi</td><td>: {{ $categories->count() }} paket</td></tr>
        ...
        <tbody>
            @forelse ($rows as $idx => $r)
                <tr>
                    ...
                    @foreach ($categories as $cat)
                        <td class="center">{{ $r['nilai'][$cat->id] ?? '—' }}</td>
                    @endforeach
                    <td class="center">{{ $r['rata'] ?? '—' }}</td>
                    <td class="center">{{ $r['status'] }}</td>
                </tr>
            @empty
                <tr><td colspan="{{ $categories->count() + 4 }}" class="center">Tidak ada data.</td></tr>
            @endforelse
        </tbody>
    </table>

    <p class="keterangan">
        Keterangan: Rata-rata dihitung dari paket evaluasi yang sudah diisi mahasiswa. Status "Lulus" berarti seluruh
        paket yang diisi telah mencapai nilai kelulusan (passing grade) masing-masing.
    </p>
