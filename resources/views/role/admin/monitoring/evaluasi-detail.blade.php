...
<div class="mb-5 flex items-start justify-between flex-wrap gap-3">
    ...
    <div class="flex items-center gap-2">
        <a href="{{ route($monBase.'.evaluasi.export-pdf', ['groupId' => $group->id]) }}" target="_blank"
        ...
        <a href="{{ route($monBase.'.evaluasi.export-excel', ['groupId' => $group->id]) }}"
            ...
            <thead>
                <tr>
                    ...
                    @foreach ($categories as $cat)
                        <th class="text-center text-[11px] font-extrabold uppercase tracking-wider text-slate-400 px-3.5 py-3 bg-slate-100 whitespace-nowrap">{{ $cat->title }}</th>
                    @endforeach
